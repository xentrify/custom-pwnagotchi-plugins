...
services that provide this. [Duck DNS](https://www.duckdns.org/) is a great one. Some routers will allow you to use a dynamic DNS provider in their settings, so you don't
have to manually update it. You can also integrate it in [home assistant](https://www.home-assistant.io/integrations/duckdns/) 
or run a [docker](https://hub.docker.com/r/linuxserver/duckdns). Or you can use something like Wireguard to have a VPN tunnel
back to your home network.

**Testing it**

Sometimes the FTP server is only accessible from the outside, you can use these tools to test it:
